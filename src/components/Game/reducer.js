...
const visitors = {
        ...
        return state.phase === phases.locking && action.payload === directions.down ?
            doLock(state) : {
                ...
                phase: landed(updatedTetromino, playfield) ?
                    phases.locking : phases.descending
            };
    },
    lock: doLock,
    clear: (state) => {
        ...
        return {
            ...
            lines,
            sfx: clearResult.linesCleared > 0 ?
                'clear' + clearResult.linesCleared : undefined,
    ...
    spawn: (state, action) => {
        ...
        const isAlive = !landed(newTetromino, playfield) && !blockedOut(newTetromino, playfield);
        return {
            ...state,
    ...
    hold: (state) => state.holdLock ? state : ({
      ...state,
